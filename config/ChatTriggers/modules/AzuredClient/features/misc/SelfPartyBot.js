...
const addUUID = (username) => request(`https://api.mojang.com/users/profiles/minecraft/${username}`).then(function (data) {
	...
	if (blacklist.uuids.includes(uuid)) {
		...
		ChatLib.chat(`§6[§bAzuredClient§6]§c Removed §6${username}§c from the blacklist.`)
	} else {
		blacklist.uuids.push(uuid)
		blacklist.igns.push(username.toLowerCase())
		ChatLib.chat(`§6[§bAzuredClient§6]§a Successfully added §6${username}§a to the blacklist.`)
	}
	blacklist.save()
}).catch(e => {
	ChatLib.chat(`§6[§bAzuredClient§6]§c Error: §f${JSON.parse(e).errorMessage}`)
})

let blacklist = new PogObject("AzuredClient", {
	uuids: [
	],
...
register('command', (ign) => {
	if (!ign) {
		if (blacklist.igns.length != 0) {
			ChatLib.chat(`§6[§bAzuredClient§6]§a Current blacklist: §f${blacklist.igns.toString()}`)
		} else ChatLib.chat(`§6[§bAzuredClient§6]§c Wrong usage. Use /botblacklist {ign} to blacklist a player.`)
		return
	}
...
register('chat', (rank, name, mode, floor) => {
	if (!config.partyCommands || Date.now() - lastTimeUsed < DELAY || blacklist.igns.includes(name.toLowerCase()) || !Array.from(numbersToText.keys()).includes(floor)) return
...
register('chat', (rank, name) => {
	if (!config.partyCommands || Date.now() - lastTimeUsed < DELAY || blacklist.igns.includes(name.toLowerCase())) return
	runCommand('pc Commands: !warp, !ptme, !inv(ite), !allinv(ite), !ping, !tps, !rp, !cf(coinflip), !dice !m{floor}, !math')
...
}).setCriteria(/^Party > (?:\[([^\]]*?)\] )?(\w{1,16})(?: [ቾ⚒])?: !inv(ite)? (.+)$/)
...
}).setCriteria(/^Party > (?:\[([^\]]*?)\] )?(\w{1,16})(?: [ቾ⚒])?: !allinv(ite)?$/)
...
}).setCriteria(/^Party > (?:\[([^\]]*?)\] )?(\w{1,16})(?: [ቾ⚒])?: (!rp|!reparty)$/)
...
register('chat', (rank, name) => {
	if (!config.partyCommands || Date.now() - lastTimeUsed < DELAY || blacklist.igns.includes(name.toLowerCase())) return
	runCommand(`pc ${name} rolled ${Math.floor(Math.random() * 2) == 0 ? 'Heads' : 'Tails'}`)
}).setCriteria(/^Party > (?:\[([^\]]*?)\] )?(\w{1,16})(?: [ቾ⚒])?: !(coinflip|cf)$/)

register('chat', (rank, name, operation) => {
	if (!config.partyCommands || Date.now() - lastTimeUsed < DELAY || blacklist.igns.includes(name.toLowerCase())) return
	runCommand(`pc ${operation} = ${eval(operation)}`)
}).setCriteria(/^Party > (?:\[([^\]]*?)\] )?(\w{1,16})(?: [ቾ⚒])?: !math ([^a-zA-Z!]+)$/)
...
register('chat', (rank, name) => {
	if (!config.partyCommands || Date.now() - lastTimeUsed < 2000 || blacklist.igns.includes(name.toLowerCase())) return
